...
| Ekip Hesapları (yönetici ekibine hesap açar + e-postadaki tek kullanımlık giriş bağlantısı) | `backend/src/modules/ekip-hesaplari/`, bağlantı `backend/src/modules/auth/giris-baglantisi.service.ts`, arayüz `apps/web/src/components/ekipHesaplari/`, açılan sayfa `apps/web/src/pages/HesapGiris.tsx` — kadro/modül kaydı mevcut tablolara yazılır; şifre e-postaya YAZILMAZ; bağlantı yalnızca kişi hiç girmemişken yeniden gönderilir; Lio `create_team_account` ile açar, şifreyi sunucu üretir ve modele hiç göstermez (migration 130) |
...
| Mobil bildirimler (FCM — WebView'de web push yok) | `backend/src/modules/notifications/fcm.ts`, cihazlar `push_cihazlari` (migration 131), kabuk tarafı `apps/web/src/lib/mobilKabuk.ts`; kurulum `docs/mobil-bildirimler.md` |
...
| Google Takvim (kullanıcının takvimi Projelio'da + Projelio'dan Google'a + Lio göreve çevirir) | `backend/src/modules/google-takvim/` — saf API/eşleme `takvim-api.ts`, gün bölme `packages/shared/src/googleTakvim.ts`, arayüz `apps/web/src/components/googleTakvim/`; kurulum ve Google doğrulaması `docs/google-takvim.md` (migration 133) |
| Arkadaşlık + kişisel duvar (iş/şirket ilişkisinden bağımsız sosyal sayfa) | `backend/src/modules/arkadaslar/` (kararlar saf: `arkadaslik-durumu.ts`), duvar paylaşımları `project-posts` içinde (`project_posts.wall_user_id`), duvar yetkisi `AccessService.assertCanViewWall`; arayüz `apps/web/src/pages/Sosyal.tsx` + ortak `FeedPanel` — `project_posts`'un users'a iki bağı var, gömmede `users!user_id(...)` ipucu ŞART (migration 134) |
...
| Mobil uygulama bildirimleri | `FCM_PROJECT_ID` + `FCM_CLIENT_EMAIL` + `FCM_PRIVATE_KEY` | `backend/.env` — eksikse telefona bildirim gitmez |
