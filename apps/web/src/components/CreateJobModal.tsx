...
export default function CreateJobModal({ onClose, onCreated }: Props) {
  ...
  return (
    <Modal title={t("Yeni iş")} onClose={onClose}>
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <label style={{ fontSize: 15, color: c.textSecondary }}>{t("Başlık")}</label>
          <input value={title} onChange={(e) => setTitle(e.target.value)} required placeholder={t("Örn. Web tasarım işleri")} style={{ width: "100%" }} />
        </div>
