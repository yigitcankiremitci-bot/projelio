...
export default function GoogleDriveCard() {
  ...
  const load = () => driveApi.status().then(setStatus).catch(() => setStatus(null));

  useEffect(() => {
    void load();
    // OneDrive kartında bağlantı kurulur/kaldırılırsa bu kartın kilitli
        ...
        <>
          <p style={{ fontSize: 15, color: c.textSecondary, margin: "0 0 12px", lineHeight: 1.5 }}>
            {status.needsReconnect
              ? "Drive erişiminiz sona ermiş. Dosyalarınıza erişmeye devam etmek için yeniden bağlanın."
              : "Projelere, görevlere ve çıktılara dosya ekleyebilmek için Drive hesabınızı bağlayın."}
          </p>
          <button
          ...
          >
            {busy ? "Yönlendiriliyor…" : status.needsReconnect ? "Yeniden bağlan" : "Drive'ı bağla"}
          </button>
        </>
      )}
