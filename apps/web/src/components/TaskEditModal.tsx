  ...
  return (
    <Modal title="Görevi düzenle" onClose={onClose} maxWidth={460}>
      <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 6, flex: 1 }}>
            <label style={{ fontSize: 12, color: c.textSecondary }}>Ekip</label>
      ...
      <div style={{ borderTop: `1px solid ${c.border}`, marginTop: 20, paddingTop: 16 }}>
        <h3 style={{ fontSize: 13, fontWeight: 500, color: c.textPrimary, margin: "0 0 10px" }}>Yorumlar</h3>

        {comments.length === 0 ? (
          <p style={{ fontSize: 12, color: c.textSecondary, margin: "0 0 12px" }}>Henüz yorum yok.</p>
        ) : (
            ...
            {comments.map((cm) => (
              <div key={cm.id} style={{ background: c.background, border: `1px solid ${c.border}`, borderRadius: 8, padding: "8px 10px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                  <span style={{ fontSize: 12, fontWeight: 500, color: c.textPrimary }}>{cm.authorName}</span>
                  <span style={{ fontSize: 11, color: c.textSecondary }}>
                    {new Date(cm.createdAt).toLocaleDateString("tr-TR")}
                  </span>
                </div>
                <p style={{ fontSize: 13, color: c.textPrimary, margin: 0, lineHeight: 1.4 }}>{cm.body}</p>
              </div>
            ))}
          </div>
        )}
        ...
        <form onSubmit={handleAddComment} style={{ display: "flex", gap: 8 }}>
          ...
          <button
            ...
            style={{ padding: "0 16px", borderRadius: 8, border: "none", background: c.primary, color: "#fff", fontSize: 13, fontWeight: 500 }}
          >
            Gönder
          </button>
        </form>
