  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        notes: { type: "string" },
        kind: { type: "string", enum: ["product", "service"], description: "Ürün mü hizmet mi; varsayılan product." },
        features: { type: "array", items: { type: "string" }, description: "Öne çıkan özellikler, kısa maddeler." },
        specs: {
          type: "array",
          items: {
            type: "object",
          ...
          description: "Teknik özellikler (ör. Ağırlık → 2,4 kg). Güncellemede listenin TAMAMINI gönder.",
        },
        warranty: { type: "string", description: "Garanti, serbest metin (ör. 2 yıl)." },
        leadTime: { type: "string", description: "Teslim / başlama süresi (ör. 3 iş günü)." },
        minStock: { type: "number", description: "Kritik stok seviyesi." },
      },
      required: ["organizationId", "name"],
    },
  },
